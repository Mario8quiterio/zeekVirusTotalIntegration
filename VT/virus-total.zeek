...
event zeek_init()
	{
	Log::create_stream(VirusTotal::LOG, [$columns=Report, $path="testing"]);
	}
...
function VirusTotal::scan_hash(f: fa_file, hash: string): Report
	...
	if ( api_key != "" &&
		...
		return when [f, copy r]( local result = ActiveHTTP::request(r) )
			{
...
#ERROR
# warning in /opt/share/zeek/policy/VT/./virus-total.zeek, lines 89-100:"when statement
# referring to locals without an explicity [] capture is deprecated"
#raw.githubusercontent.com/zeek/zeek/master/CHANGES
